...
| Every §6.4 duration is read from `app_settings` inside its **declared** bounds and under the absolute ceiling — no compiled-in default, and no dependent duration may outrun `link.absolute_expiry_days` | `AppSettingMapper.readStored` takes bounds from `min_value`/`max_value` as data, never from a code constant; `crossFieldErrors()` carries the five relative rules and runs on the **read** path as well as the write; `AppSettingsRepository` is the only port returning `DomainResult`, so an unreadable row cannot be defaulted away | §6.4, §8.10, SEC-39 |
