...
> ### SEC-41 — this ticket is gated: a rendered email body carrying a live PIN already exists (2026-09-18, [ERT-300 review](../2026-09-18-ert-300-review.md))
>
> **Do not write the issuance path until this is decided.** Four documents say the same thing
> absolutely — PRD §12 (*"never sent by email at all"*), PRD §5 (*"It is never sent by email"*),
> architecture §12 invariant 4, and `CLAUDE.md` invariant 4. The code already holds the opposite:
>
> - `Notifier.sendRecoveryPin(to: EmailAddress, employee: Employee, pin: AccessPin)`
> - `NotificationMessages.recoveryPin` renders `${pin.value}` into a plaintext body
> - `OutboxNotifier.sendRecoveryPin` queues it under `NotificationKind.RECOVERY_PIN`
>
> Nothing calls it. **This ticket is the first caller it would get**, and an engineer writing it finds
> the machinery pre-built and named for the job, with a port KDoc stating a materially weaker rule
> than all four documents: *"no email **other than a recovery notification** ever contains the PIN"*.
>
> The `storesBody = false` half is fine — the body is never persisted, so invariant 4's "no stored
> artefact holds a live PIN" holds. This is about transmission only.
>
> It matters because of SEC-03: the recovery PIN exists precisely because the invitation may have gone
> to the wrong person, and emailing the recovery credential to the same address returns the system to
> the state the PIN was introduced to escape. §12's residual-risk acceptance for link-only portal
> access is written assuming the recovery path is out-of-band.
>
> - [ ] Given the §12 rule stands, then **no `Notifier` method accepts an `AccessPin`** and
>       `recoveryPin` renders a body naming no credential — the PIN reaches exactly the one-time HR
>       response and nothing else, which is what makes the rule unrepresentable to break
> - [ ] Given instead that a deliberate exception exists, then **§12, PRD §5, architecture §12 and
>       `CLAUDE.md` are amended to carry it** — the port KDoc is not where that decision lives
