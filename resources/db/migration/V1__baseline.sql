...
create table departments (id uuid primary key, "name" varchar(128) not null);
alter table departments add constraint departments_name_unique unique ("name");
create table employment_types (id uuid primary key, "name" varchar(128) not null);
alter table employment_types add constraint employment_types_name_unique unique ("name");
create table requirement_templates (id uuid primary key, "name" varchar(256) not null, instructions text not null, is_required boolean not null, expires boolean default false not null, validity_months int null, renewal_lead_days int null, is_active boolean default true not null, sort_order int default 0 not null);
create table template_assignments (employment_type_id uuid, requirement_template_id uuid, constraint pk_template_assignments primary key (employment_type_id, requirement_template_id), constraint fk_template_assignments_employment_type_id__id foreign key (employment_type_id) references employment_types(id) on delete restrict on update restrict, constraint fk_template_assignments_requirement_template_id__id foreign key (requirement_template_id) references requirement_templates(id) on delete restrict on update restrict);
create table employees (id uuid primary key, first_name varchar(128) not null, middle_initial varchar(8) null, last_name varchar(128) not null, department_id uuid not null, "position" varchar(256) not null, employment_type_id uuid not null, email varchar(320) not null, packet_status varchar(32) not null, submitted_at timestamp null, submitted_by_hr boolean default false not null, attestation_version varchar(32) null, attested_at timestamp null, attested_ip varchar(64) null, originals_sighted_at timestamp null, originals_sighted_by varchar(128) null, anomaly_flags varchar(512) default '' not null, completed_at timestamp null, created_at timestamp not null, created_by varchar(128) not null, constraint fk_employees_department_id__id foreign key (department_id) references departments(id) on delete restrict on update restrict, constraint fk_employees_employment_type_id__id foreign key (employment_type_id) references employment_types(id) on delete restrict on update restrict);
create index employees_email on employees (email);
create table employee_requirements (id uuid primary key, employee_id uuid not null, template_id uuid not null, name_snapshot varchar(256) not null, is_required_snapshot boolean not null, status varchar(32) not null, rejection_count int default 0 not null, constraint fk_employee_requirements_employee_id__id foreign key (employee_id) references employees(id) on delete restrict on update restrict, constraint fk_employee_requirements_template_id__id foreign key (template_id) references requirement_templates(id) on delete restrict on update restrict);
create table submissions (id uuid primary key, employee_requirement_id uuid not null, version int not null, file_key varchar(512) not null, original_filename varchar(512) not null, mime_type varchar(128) not null, size_bytes bigint not null, uploaded_at timestamp not null, status varchar(32) not null, valid_from timestamp null, valid_until timestamp null, reviewed_by varchar(128) null, reviewed_at timestamp null, rejection_reason text null, is_current boolean default true not null, constraint fk_submissions_employee_requirement_id__id foreign key (employee_requirement_id) references employee_requirements(id) on delete restrict on update restrict);
create table upload_links (id uuid primary key, employee_id uuid not null, token_hash varchar(256) not null, pin_hash varchar(256) not null, scope varchar(512) default 'ALL' not null, status varchar(32) not null, issued_at timestamp not null, expires_at timestamp not null, idle_expires_at timestamp null, extended_count int default 0 not null, failed_pin_count int default 0 not null, locked_until timestamp null, warned_at timestamp null, revoked_at timestamp null, revoked_reason text null, constraint fk_upload_links_employee_id__id foreign key (employee_id) references employees(id) on delete restrict on update restrict);
alter table upload_links add constraint upload_links_token_hash_unique unique (token_hash);
create table portal_sessions (id uuid primary key, upload_link_id uuid not null, token_hash varchar(256) not null, started_at timestamp not null, expires_at timestamp not null, ip varchar(64) not null, user_agent varchar(512) not null, ended_at timestamp null, constraint fk_portal_sessions_upload_link_id__id foreign key (upload_link_id) references upload_links(id) on delete restrict on update restrict);
alter table portal_sessions add constraint portal_sessions_token_hash_unique unique (token_hash);
create table portal_access_logs (id uuid primary key, upload_link_id uuid not null, session_id uuid null, "timestamp" timestamp not null, ip varchar(64) not null, user_agent varchar(512) not null, "action" varchar(32) not null, outcome varchar(32) not null, constraint fk_portal_access_logs_upload_link_id__id foreign key (upload_link_id) references upload_links(id) on delete restrict on update restrict, constraint fk_portal_access_logs_session_id__id foreign key (session_id) references portal_sessions(id) on delete restrict on update restrict);
...
create table audit_logs (id uuid primary key, actor varchar(128) not null, "action" varchar(64) not null, entity varchar(64) not null, entity_id uuid not null, "timestamp" timestamp not null, metadata text default '{}' not null);
